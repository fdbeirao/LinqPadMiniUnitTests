...
{
	public static void RunTests(bool throwIfAnyTestFails = true)
	{
		var assemblyClasses = Assembly.GetCallingAssembly().GetTypes();

		var allMethodsWithTestAttribute = assemblyClasses.SelectMany(type => type.GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(method => method.IsDefined(typeof(TestAttribute))));

		var testResults = new List<TestResult> { };
		foreach (var method in allMethodsWithTestAttribute)
		{
			var testClass = method.DeclaringType;

			var testName = string.Format("{0}.{1}", testClass.Name, method.Name);
			...
			testResults.Add(new TestResult
			{
				...
				TestCode = (Action)Delegate.CreateDelegate(typeof(Action), Activator.CreateInstance(testClass), method.Name),
